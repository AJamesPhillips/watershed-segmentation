...
describe("build_graph_of_watershed_connections", () =>
{
    test("it should handle a simple case", () =>
    {
        const watershed: Watersheds = {
            ...
            watershed_count: 2,
            vertices: [
                // A linear strip of land 1 high, 3 wide with a peak in the middle
        ...
        expect(connections.get(0)).toEqual(new Map([
            // [-1, { x: 0, y: 0, z: -2, id: -1 }],
            [1, { x: 1, y: 0, z: 3, id: 1 }]
        ]))
        expect(connections.get(1)).toEqual(new Map([
            [0, { x: 1, y: 0, z: 3, id: 0 }]]))
    })
    ...
    test("it should handle many watersheds and exits", () =>
    {
        const watershed: Watersheds = {
            ...
            watershed_count: 4,
            vertices: [
                // A linear strip of land 1 high, 7 wide with alternating peaks and valleys
                { z: 0, watershed_ids: new Set([0         ]) },
        ...
        expect(connections.get(1)).toEqual(new Map([
            ...
            [0, { x: 1, y: 0, z: 7, id: 0 }],
            [2, { x: 3, y: 0, z: 5, id: 2 }]]))
        expect(connections.get(2)).toEqual(new Map([
            // [-2, { x: 3, y: 0, z: -3, id: -2 }],
            [1, { x: 3, y: 0, z: 5, id: 1 }],
            [3, { x: 5, y: 0, z: 9, id: 3 }]]))
        expect(connections.get(3)).toEqual(new Map([
            // [-3, { x: 6, y: 0, z: -4, id: -3 }],
            [2, { x: 5, y: 0, z: 9, id: 2 }]]))
    })
    ...
    test("it should handle 2d watersheds and internal exit", () =>
    {
        const watershed: Watersheds = {
            ...
            watershed_count: 2,
            vertices: [
                // A ridge running north to south and getting lower in the middle
                // This helps us to test that the initial lowest connection
                // between two watersheds is replaced by a new lower connection
                // between two watersheds
                { z: 0, watershed_ids: new Set([0   ]) },
                { z: 2, watershed_ids: new Set([0, 1]) },
                { z: 0, watershed_ids: new Set([   1]) },
        ...
        const exits: Exit[] = [
            // An internal exit like a cave system at the south of the ridge
            { x: 1, y: 2, z: -2, id: -1 },
        ]
